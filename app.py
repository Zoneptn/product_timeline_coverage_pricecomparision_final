...
Sidebar switch between four independent views, each in its own module:
  ...
  chemical_analysis_view.py -> "Chemical Analysis"     (reads crop_timeline.xlsx;
                                chemical x weed/pest/disease efficiency heatmap)
...
helpers) live in shared.py. Workbook loaders shared by more than one
view live in their own data_*.py module: data_threat.py (crop_timeline.xlsx,
